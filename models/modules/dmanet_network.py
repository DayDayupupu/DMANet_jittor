...
class DMANet(nn.Module):
    ...
    def execute(self, inputs, prev_states, prev_features):
        states = list()
        agg_features = list()
        # print("inputs[0][0][1]",inputs[0][0][0].shape)
        ...
        for pos_x, neg_x in zip(pos_x_list, neg_x_list):  # each batch
            ...
            # print("Voxel features shapes:", pos_voxel_features.shape, neg_voxel_features.shape)
            ...
            # print("After squeeze and permute:", pos_voxel_features.shape, neg_voxel_features.shape)

            # spatial_feature input size
            pos_spatial_features = self.middle_feature_extractor(pos_voxel_features, pos_coors.astype(jt.int32))
            neg_spatial_features = self.middle_feature_extractor(neg_voxel_features, neg_coors.astype(jt.int32))
            pos_spatial_feature_list.append(pos_spatial_features), neg_spatial_feature_list.append(neg_spatial_features)

        pos_spatial_feature = jt.concat(pos_spatial_feature_list, dim=0)
        neg_spatial_feature = jt.concat(neg_spatial_feature_list, dim=0)
        # print(pos_spatial_feature.shape)
        spatial_feature = jt.concat([pos_spatial_feature, neg_spatial_feature], dim=1)
        ...
        if prev_features is None:
            ...
            for idx, (curr, prev) in enumerate(zip(fpn_features, prev_features)):
                ...
                batch_agg_feat = jt.concat([b for b in batch_list], dim=0)
        ...
        regression = jt.concat([self.regressionModel(s+l+t) for s, l, t in zip(short_features, long_features, temporal_feature)], dim=1)
        classification = jt.concat([self.classificationModel(s+l+t) for s, l, t in zip(short_features, long_features, temporal_feature)], dim=1)
...
class ClassificationModel(nn.Module):
    ...
    def __init__(self, num_features_in, num_anchors=3*5, num_classes=80, prior=0.01, feature_size=256):
        ...
        self.output = nn.Conv(feature_size, num_anchors * num_classes, kernel_size=3, padding=1)
